...
extern ImFont* LexendLight;
extern ImFont* LexendRegular;
extern ImFont* LexendRegularTabs;
...
extern ImFont* Tabsicon;
bool ImGui::Renderingtab(const char* icon2, const char* icon, const char* label, bool selected)
{
    ...
    ImVec2 size2 = { 44, 33 };
    const ImRect bb2(pos, ImVec2(pos.x + size2.x, pos.y + size2.y));
    ...
    GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Max.x, bb.Max.y), ImColor(24, 25, 27, int(255 * GetStyle().Alpha)), 4);
    GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 55, bb.Min.y + 6), ImColor(170, 170, 170, int(255 * GetStyle().Alpha)), icon);
    GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 195, bb.Min.y + 6), ImColor(170, 170, 170, int(255 * GetStyle().Alpha)), label);

    ImGui::PushFont(Tabsicon);
    GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 14, bb.Min.y + 2), ImColor(170, 170, 170, int(255 * GetStyle().Alpha)), icon2);
    ImGui::PopFont();

    if (selected)
    {
        GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Max.x, bb.Max.y), ImColor(29, 30, 34, int(255 * it_filled->second)), 4);
        {
            GetWindowDrawList()->AddRectFilled(ImVec2(bb2.Min.x + 10, bb2.Min.y - 3), ImVec2(bb2.Max.x, bb2.Max.y), ImColor(19, 22, 27, int(255 * it_filled->second)), 4);
        }

        GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 195, bb.Min.y + 6), ImColor(145, 222, 52, int(255 * it_filled->second)), label);
        GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 55, bb.Min.y + 6), ImColor(145, 222, 52, int(255 * it_filled->second)), icon);

        ImGui::PushFont(Tabsicon);
        GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 14, bb.Min.y + 2), ImColor(145, 222, 52, int(255 * GetStyle().Alpha)), icon2);
        ImGui::PopFont();
    }
...
bool ImGui::Renderingsubtab(const char* icon, bool selected)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x + 5, bb.Max.y - 12), ImVec2(bb.Min.x + 47, bb.Max.y - 10), ImColor(145, 222, 52, int(255 * it_filled->second)));
    GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 5, bb.Min.y + 4), ImColor(170, 170, 170, int(255 * GetStyle().Alpha)), icon);

    if (selected)
    {
        GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 5, bb.Min.y + 4), ImColor(255, 255, 255, int(255 * it_filled->second)), icon);
    }
...
bool ImGui::MenuTabEx(const char* label, const bool active, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    window->DrawList->AddRectFilled(ImVec2(pos.x, pos.y + size_arg.y - 2), ImVec2(pos.x + size_arg.x, pos.y + size_arg.y), ImColor(145, 222, 52, int(255 * it_active->second)));
    window->DrawList->AddRectFilledMultiColor(ImVec2(pos.x, pos.y + size_arg.y - 15), ImVec2(pos.x + size_arg.x, pos.y + size_arg.y), ImColor(145, 222, 52, 0), ImColor(145, 222, 52, 0),
        ImColor(145, 222, 52, int(100 * it_active->second)), ImColor(145, 222, 52, int(100 * it_active->second)));
...
bool ImGui::KeyBind(const char* label, KeyBind_t* k, ImGuiKeyBindFlags flags)
{
    ...
    createkeybind(label, KeyNames[k->id]);

    ImGuiKeyBind* keybind = findkeybind(label);
    ImVec2 key_size = CalcTextSize(keybind->key.c_str(), NULL, true);

    static bool popup_open;
    const float w = GetWindowSize().x - style.WindowPadding.x;
    const int frame_w = 65;
    const ImRect frame_bb(window->DC.CursorPos, ImVec2(window->Pos.x + w, window->DC.CursorPos.y + label_size.y + style.FramePadding.y * 2.0f));

    const ImRect clip_rect_bb(ImVec2(frame_bb.Max.x - frame_w, frame_bb.Min.y - 1), frame_bb.Max);
    ...
    const bool hovered = ItemHoverable(clip_rect_bb, id);
    ...
    it_open->second = ImClamp(it_open->second + (2.f * ImGui::GetIO().DeltaTime * (popup_open ? 1.f : -5.f)), 0.f, 1.f);
    if (!popup_open)
        it_open->second = 0.01f;

    if (!(flags & ImGuiKeyBindFlags_NoSettings) && g.ActiveId != id)
    {
        std::string name_tooltip = label; name_tooltip += "##Tooltip";
        ...
        SetNextWindowSize(ImVec2(frame_w * it_open->second, 70 * it_open->second));
        if (ItemsToolTipBeginKeyBind(name_tooltip.c_str(), hovered, frame_bb.Max - ImVec2(frame_w * it_open->second, -2)))
        {
            popup_open = true;
            PushStyleVar(ImGuiStyleVar_ItemSpacing, ImVec2(style.ItemSpacing.x, 0));
            if (Selectable("Off", k->type == 0, ImGuiSelectableFlags_DontClosePopups, ImVec2(frame_w, 15)))
                k->type = 0;
            if (Selectable("Hold", k->type == 1, ImGuiSelectableFlags_DontClosePopups, ImVec2(frame_w, 15)))
                k->type = 1;
            if (Selectable("Toggle", k->type == 2, ImGuiSelectableFlags_DontClosePopups, ImVec2(frame_w, 15)))
                k->type = 2;
            if (Selectable("Always", k->type == 3, ImGuiSelectableFlags_DontClosePopups, ImVec2(frame_w, 15)))
                k->type = 3;
            PopStyleVar();
            ItemsToolTipEnd();
        }
    ...
    if (k->id != 0 && g.ActiveId != id)
    {
        keybind->key = KeyNames[k->id];
    }
    ...
    key_size = CalcTextSize(keybind->key.c_str(), NULL, true);
    ...
    it_hovered->second = ImClamp(it_hovered->second + (1.f * ImGui::GetIO().DeltaTime * (hovered || g.ActiveId == id || popup_open ? 1.f : -1.f)), 0.0f, 1.0f);
    it_hovered->second *= style.Alpha;
    ...
    it_active->second = ImClamp(it_active->second + (1.f * ImGui::GetIO().DeltaTime * (g.ActiveId == id || popup_open ? 1.f : -1.f)), 0.0f, 1.0f);
    it_active->second *= style.Alpha;
    ...
    window->DrawList->AddRectFilled(ImVec2(frame_bb.Max.x - frame_w, frame_bb.Min.y), frame_bb.Max, ImColor(15, 15, 15, int(255 * fill)), style.FrameRounding);
    window->DrawList->AddRect(ImVec2(frame_bb.Max.x - frame_w, frame_bb.Min.y), frame_bb.Max, ImColor(55, 54, 57, int(255 * fill)), style.FrameRounding);
    window->DrawList->AddRect(ImVec2(frame_bb.Max.x - frame_w, frame_bb.Min.y), frame_bb.Max, ImColor(145, 222, 52, int(255 * it_hovered->second)), style.FrameRounding);

    PushStyleColor(ImGuiCol_Text, ImColor(130, 130, 130, int(255 * fill)).operator ImVec4());
    RenderTextClipped(clip_rect_bb.Min, clip_rect_bb.Max, keybind->key.c_str(), NULL, &key_size, ImVec2(0.5f, 0.5f), &clip_rect_bb);
    ...
    if (label_size.x > 0 && !(flags & ImGuiKeyBindFlags_OnItem))
        RenderTextClipped(frame_bb.Min + style.FramePadding, frame_bb.Max - style.FramePadding, label, NULL, &label_size, ImVec2(0.f, 0.5f), &frame_bb);

    return value_changed;
}

bool ImGui::MenuChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags, float rounding)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* parent_window = g.CurrentWindow;

    flags |= ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_ChildWindow | ImGuiWindowFlags_NoBackground;
    ...
    bool ret = Begin(title, NULL, flags | ImGuiWindowFlags_NoTitleBar);
    ...
    static float fill;
    fill = ImClamp(fill + (2.7f * ImGui::GetIO().DeltaTime), 0.0f, 1.f);
    fill *= g.Style.Alpha;

    auto p = parent_window->DC.CursorPos;
    parent_window->DrawList->AddRectFilled(p, p + size_arg, ImColor(26, 28, 30, int(255 * fill)));
...
bool ImGui::MenuChild(const char* str_id, const ImVec2& size_arg, bool border, ImGuiWindowFlags extra_flags, float rounding)
{
    ImGuiWindow* window = GetCurrentWindow();
    return MenuChildEx(str_id, window->GetID(str_id), size_arg, border, extra_flags, rounding);
}
...
void ImGui::AddCircleImageFilled(ImTextureID user_texture_id, const ImVec2& centre, float radius, ImU32 col, int num_segments)
{
    ...
    ShadeVertsLinearUV(window->DrawList, vert_start_idx, vert_end_idx, ImVec2(centre.x - radius, centre.y - radius), ImVec2(centre.x + radius, centre.y + radius), ImVec2(0, 0), ImVec2(1, 1), true);
...
void ImGui::MultiCombo(const char* label, bool* combos, const char* items[], int items_count)
{
    ...
    for (int i = 0, j = 0; i < items_count; i++)
    {
        if (combos[i])
        {
            ...
            if (j > 4)
                preview = vec.at(0) + _(", ") + vec.at(1) + _(", ") + vec.at(2) + _(", ") + vec.at(3) + _(", ") + vec.at(4) + _(", ...");
            else if (j)
                preview += _(", ") + (std::string)items[i];
            else
                preview = items[i];
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    // Render
    RenderNavHighlight(bb, id);
    window->DrawList->AddRectFilled(bb.Min, bb.Max, ImColor(97, 174, 248, int(255 * fill)), style.FrameRounding);

    PushStyleColor(ImGuiCol_Text, ImColor(255, 255, 255, int(255 * fill)).operator ImVec4());
...
void ImGui::Scrollbar(ImGuiAxis axis)
{
    ...
    it_active->second = ImClamp(it_active->second + (2.f * ImGui::GetIO().DeltaTime * (held ? 1.f : -1.f)), 0.0f, 1.0f);
    it_active->second *= style.Alpha;
    ...
    window->DrawList->AddRectFilled(grab_rect.Min, grab_rect.Max, ImColor(45, 45, 45, int(255 * fill)), style.ScrollbarRounding);
    window->DrawList->AddRectFilled(grab_rect.Min, grab_rect.Max, ImColor(145, 222, 52, int(100 * it_hovered->second)), style.ScrollbarRounding);
    window->DrawList->AddRectFilled(grab_rect.Min, grab_rect.Max, ImColor(145, 222, 52, int(255 * it_active->second)), style.ScrollbarRounding);
}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, ImColor(26, 28, 30, int(255 * fill)), style.FrameRounding);

    ImU32 check_col = ImColor(97, 174, 248);
    const float pad = ImMax(1.0f, (square_sz / 5.f));

    if(*v)
    RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(pad, pad), check_col, square_sz - pad * 2.0f);

    ImGui::PushFont(LexendRegularTabs);
    {
        if (g.LogEnabled)
            LogRenderedText(&total_bb.Min, *v ? "[x]" : "[ ]");

        PushStyleColor(ImGuiCol_Text, ImColor(255, 255, 255, int(255 * fill)).operator ImVec4());
        RenderText(ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y - 2), label);

        PushStyleColor(ImGuiCol_Text, ImColor(255, 255, 255, int(100 * it_hovered->second)).operator ImVec4());
        RenderText(ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y - 2), label);

        PushStyleColor(ImGuiCol_Text, ImColor(97, 174, 248, int(255 * it_active->second)).operator ImVec4());
        RenderText(ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y - 2), label);
    }
    ImGui::PopFont();
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float w = GetWindowWidth() - style.WindowPadding.x * 2;
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, 20));
    ...
    it_hovered->second = ImClamp(it_hovered->second + (1.f * ImGui::GetIO().DeltaTime * (hovered || popup_open ? 1.f : -1.5f)), 0.0f, 1.0f);
    it_hovered->second *= style.Alpha;
    ...
    it_open->second = ImClamp(it_open->second + (3.f * GetIO().DeltaTime * (popup_open ? 1.f : -1.f)), 0.f, 1.f);
    it_open->second *= style.Alpha;
    ...
    it_active->second = ImClamp(it_active->second + (1.f * ImGui::GetIO().DeltaTime * (popup_open ? 1.f : -1.f)), 0.0f, 1.0f);
    it_active->second *= style.Alpha;
    ...
    window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(15, 15, 15, int(255 * fill)), style.FrameRounding);
    window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImColor(53, 54, 57, int(255 * fill)), style.FrameRounding);
    window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImColor(215, 10, 3, int(255 * it_hovered->second)), style.FrameRounding);

    ImGui::PushFont(LexendLight);
    {
        PushStyleColor(ImGuiCol_Text, ImColor(255, 255, 255, int(255 * fill)).operator ImVec4());
        RenderArrow(ImVec2(frame_bb.Max.x - arrow_size - style.FramePadding.x * 3, frame_bb.Min.y + 3), popup_open ? ImGuiDir_Up : ImGuiDir_Down);
        if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
        {
            PushStyleColor(ImGuiCol_Text, ImColor(130, 130, 130, int(255 * fill)).operator ImVec4());
            RenderTextClipped(ImVec2(frame_bb.Min.x + style.FramePadding.x + style.ItemInnerSpacing.x, frame_bb.Min.y - 2), frame_bb.Max - style.FramePadding, preview_value, NULL, NULL, ImVec2(0.0f, 0.5f));
        ...
        if (label_size.x > 0)
            RenderText(ImVec2(frame_bb.Min.x, frame_bb.Min.y - g.Font->FontSize - 2), label);
        PopStyleColor();
    }
    ImGui::PopFont();
    ...
    if (it_open->second < 0.99f) {
        PushStyleVar(ImGuiStyleVar_ScrollbarSize, 0.000001f);
        style.ScrollbarRounding = 0;
    }
    else
    {
        PushStyleVar(ImGuiStyleVar_ScrollbarSize, 3.f);
        if (style.ScrollbarRounding < 2)
            style.ScrollbarRounding += 0.02f;
    }
    PushStyleVar(ImGuiStyleVar_Alpha, it_open->second);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, float power)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, 6));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    ...
    window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(15, 15, 15, int(255 * fill)), style.FrameRounding);
    window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImColor(55, 54, 57, int(255 * fill)), style.FrameRounding);
    window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(max(frame_bb.Min.x + 14, grab_bb.Max.x * slide + 2), frame_bb.Max.y), ImColor(145, 222, 52, int(255 * fill)), style.FrameRounding);
    window->DrawList->AddRectFilled(ImVec2(max(frame_bb.Min.x + 7, grab_bb.Max.x * slide - 5), frame_bb.Min.y - 2),
        ImVec2(max(frame_bb.Min.x + 14, grab_bb.Max.x * slide + 2), frame_bb.Max.y + 2), ImColor(145, 222, 52, int(255 * fill * 1.5f)), style.FrameRounding);
    ...
    ImGui::PushFont(LexendLight);
    {
        ...
        RenderText(ImVec2(frame_bb.Max.x - label_buffer_size.x, frame_bb.Min.y - g.Font->FontSize - 2), value_buf, value_buf_end);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float w = GetWindowWidth() - style.WindowPadding.x * 2;
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, 35));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? (style.ItemInnerSpacing.x + label_size.x) : 0.0f, 0.0f));
    ...
    it_active->second = ImClamp(it_active->second + (1.f * ImGui::GetIO().DeltaTime * (g.ActiveId == id ? 1.f : -1.f)), 0.0f, 1.0f);
    it_active->second *= style.Alpha;
    ...
    if (!is_multiline)
    {
        ...
        window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImColor(97, 174, 248, int(255 * fill)), style.FrameRounding);
    }

    const ImVec4 clip_rect(frame_bb.Min.x, frame_bb.Min.y, frame_bb.Max.x, frame_bb.Max.y); // Not using frame_bb.Max because we have adjusted size
    ...
    if (render_cursor || render_selection)
    {
...

        {
            ...
            if (is_multiline)
                text_size = ImVec2(w, line_count * g.FontSize);
        }
        ...
        if (render_cursor && state->CursorFollow)
        {
            ...
            {
                const float scroll_increment_x = w * 0.25f;
                if (cursor_offset.x < state->ScrollX)
                    state->ScrollX = (float)(int)ImMax(0.0f, cursor_offset.x - scroll_increment_x);
                else if (cursor_offset.x - w >= state->ScrollX)
                    state->ScrollX = (float)(int)(cursor_offset.x - w + scroll_increment_x);
            }
            else
            {
                state->ScrollX = 0.0f;
            ...
            if (is_multiline)
            {
                ...
                else if (cursor_offset.y - 20 >= scroll_y)
                    scroll_y = cursor_offset.y - 20;
        ...
        if (render_selection)
        {
            ...
            float bg_offy_up = is_multiline ? 0.0f : 2.0f;    // FIXME: those offsets should be part of the style? they don't play so well with multi-line selection.
            float bg_offy_dn = is_multiline ? 0.0f : 19.0f;
        ...
        ImGui::PushFont(LexendRegularTabs);
        {
            ...
            {
                PushStyleColor(ImGuiCol_Text, ImColor(255, 255, 255, int(255 * fill)).operator ImVec4());
                RenderTextClipped(ImVec2(frame_bb.Min.x + style.FramePadding.x + style.ItemInnerSpacing.x, frame_bb.Min.y - 2), frame_bb.Max - style.FramePadding, buf_display, buf_display_end, NULL, ImVec2(0.0f, 0.5f));
        ...
        if (render_cursor)
        {
            ...
            bool cursor_is_visible = (!g.IO.ConfigInputTextCursorBlink) || (state->CursorAnim <= 5.0f) || ImFmod(state->CursorAnim, 1.20f) <= 0.80f;
            ImVec2 cursor_screen_pos = draw_pos + cursor_offset - draw_scroll + ImVec2(5, 10);
    ...
    else
    {
        ...
        if (is_multiline)
            text_size = ImVec2(w, InputTextCalcTextLenAndLineCount(buf_display, &buf_display_end) * g.FontSize); // We don't need width
        ...
        ImGui::PushFont(LexendRegularTabs);
        {
            if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
            {
                PushStyleColor(ImGuiCol_Text, ImColor(128, 128, 128, int(255 * fill)).operator ImVec4());
                RenderTextClipped(ImVec2(frame_bb.Min.x + style.FramePadding.x + style.ItemInnerSpacing.x, frame_bb.Min.y - 2), frame_bb.Max - style.FramePadding, buf_display, buf_display_end, NULL, ImVec2(0.0f, 0.5f));
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    it_open->second.mult = ImClamp(it_open->second.mult + (2.f * GetIO().DeltaTime * (it_open->second.play ? 1.f : -1.f)), 0.01f, 1.f);
    ...
    ImGui::PushFont(LexendLight);
    {
        ...
        if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
        {
            ...
            SetCursorPosX(GetWindowSize().x - style.WindowPadding.x - square_sz);
            ...
            if (BeginPopup("picker", ImGuiWindowFlags_NoMove))
            {
                ...
                PushItemWidth(min(it_open->second.mult, 1.f) * square_sz * 12.0f); // Use 256 + bar sizes?
...
static float color_buffer[4] = { 1.f, 1.f, 1.f, 1.f };
static bool show_color[2] = { false, false };
static float alpha_here_[2] = { 0, 0 };
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    float sv_cursor_rad = value_changed_sv ? 5.0f : 4.0f;
    draw_list->AddCircleFilled(sv_cursor_pos, sv_cursor_rad, col32_no_alpha, 12);
    draw_list->AddCircle(sv_cursor_pos, sv_cursor_rad+1, IM_COL32(45,45,45,45), 12);
    ...
    SetCursorPosY(GetCursorPosY() - style.WindowPadding.y);

    auto s = window->Size;
    auto w = (s.x - style.WindowPadding.x * 3) / 2;
    if (Button("Copy", ImVec2(w, 20)))
    {
        show_color[0] = true;
        memcpy(color_buffer, col, components * sizeof(float));
    }
    SameLine(0, style.WindowPadding.x);
    if (Button("Paste", ImVec2(w, 20)))
    {
        show_color[1] = true;
        memcpy(col, color_buffer, components * sizeof(float));
    }
    alpha_here_[0] = ImClamp(alpha_here_[0] + (1.f * ImGui::GetIO().DeltaTime * (show_color[0] ? 1.5f : -1.f)), 0.0f, 1.f);
    alpha_here_[1] = ImClamp(alpha_here_[1] + (1.f * ImGui::GetIO().DeltaTime * (show_color[1] ? 1.5f : -1.f)), 0.0f, 1.f);

    ImColor end_color = ImColor(color_buffer[0], color_buffer[1], color_buffer[2]);
    if (alpha_here_[0] > 0.f)
    {
        PushStyleVar(ImGuiStyleVar_Alpha, alpha_here_[0]);
        ColorTooltip("Copied", end_color, alpha_here_[0]);
        PopStyleVar();
    }
    if (alpha_here_[1] > 0.f)
    {
        PushStyleVar(ImGuiStyleVar_Alpha, alpha_here_[1]);
        SetCursorPosX(GetCursorPosX() + 80);
        ColorTooltip("Pasted", end_color, alpha_here_[1]);
        PopStyleVar();
    }

    if (alpha_here_[0] == 1.f)
        show_color[0] = false;
    if (alpha_here_[1] == 1.f)
        show_color[1] = false;
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size)
{
    ...
    it_active->second = ImClamp(it_active->second + (2.f * ImGui::GetIO().DeltaTime), 0.0f, 1.0f);
    it_active->second *= g.Style.Alpha;

    window->DrawList->AddRectFilled(bb.Min, bb.Max, ImColor(15, 15, 15, int(255 * it_active->second)), g.Style.FrameRounding);
    window->DrawList->AddRect(bb.Min, bb.Max, ImColor(45, 45, 45, int(255 * it_active->second)), g.Style.FrameRounding);
    ...
    ImColor col_rgb = ImColor(col.x, col.y, col.z, col.w * it_active->second);
    if (flags & ImGuiColorEditFlags_InputHSV)
        ColorConvertHSVtoRGB(col_rgb.Value.x, col_rgb.Value.y, col_rgb.Value.z, col_rgb.Value.x, col_rgb.Value.y, col_rgb.Value.z);

    ImColor col_rgb_without_alpha(col_rgb.Value.x, col_rgb.Value.y, col_rgb.Value.z, it_active->second);
    float grid_step = ImMin(size.x, size.y) / 2.99f;
    float rounding = ImMin(g.Style.FrameRounding, grid_step * 0.5f);
    ImRect bb_inner = ImRect(bb.Min + ImVec2(1, 1), bb.Max - ImVec2(1, 1));
    float off = -0.75f; // The border (using Col_FrameBg) tends to look off when color is near-opaque and rounding is enabled. This offset seemed like a good middle ground to reduce those artifacts.
    bb_inner.Expand(off);
    window->DrawList->AddRectFilledMultiColor(bb_inner.Min, bb_inner.Max, col_rgb_without_alpha, col_rgb, col_rgb, col_rgb_without_alpha);
    window->DrawList->AddInvertedRectFilled(bb_inner.Min, bb_inner.Max, ImColor(15, 15, 15, int(255 * it_active->second)), g.Style.FrameRounding);
    RenderNavHighlight(bb, id);
    ...
    it_hovered->second = ImClamp(it_hovered->second + (1.f * ImGui::GetIO().DeltaTime * (hovered || g.ActiveId == id ? 1.f : -1.5f)), 0.0f, 1.0f);
    it_hovered->second *= g.Style.Alpha;

    window->DrawList->AddRect(bb.Min, bb.Max, ImColor(145, 222, 52, int(255 * it_hovered->second)), rounding);
...
void ImGui::ColorTooltip(const char* text, ImColor color, float alpha)
{
    PushStyleVar(ImGuiStyleVar_WindowBorderSize, 0);
    PushStyleVar(ImGuiStyleVar_PopupBorderSize, 0);
    PushStyleColor(ImGuiCol_WindowBg, ImColor(15, 15, 15, int(alpha * 255.f)).operator ImVec4());
    PushStyleColor(ImGuiCol_Text, ImColor(255, 255, 255, int(alpha * 255.f)).operator ImVec4());

    SetNextWindowPos(GetWindowPos() + GetCursorPos() + GetStyle().WindowPadding);
    BeginTooltipEx(0, true);

    ColorButton("", ImVec4(color.Value.x, color.Value.y, color.Value.z, alpha)); SameLine();
    Text(text);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    // Render
    window->DrawList->AddRectFilled(bb.Min, ImVec2(bb.Min.x + 4, bb.Max.y), ImColor(145, 222, 52, int(255 * it_active->second)), 10.f, ImDrawCornerFlags_Right);
    ...
    PushStyleColor(ImGuiCol_Text, ImColor(75, 75, 75, int(255 * fill)).operator ImVec4());
    RenderTextClipped(ImVec2(bb_inner.Min.x + style.WindowPadding.x, bb_inner.Min.y), bb_inner.Max, label, NULL, &label_size, style.SelectableTextAlign, &bb);
