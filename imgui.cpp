...
ImGuiStyle::ImGuiStyle()
{
    ...
    WindowPadding = ImVec2(5, 5);      // Padding within a window
...
void ImGui::RenderArrow(ImVec2 p_min, ImGuiDir dir, float scale)
{
    ...
    case ImGuiDir_Down:
        ...
        a = ImVec2(+0.00f,+0.350f) * r;
        b = ImVec2(-0.666f,-0.350f) * r;
        c = ImVec2(+0.666f,-0.350f) * r;
        break;
    case ImGuiDir_Left:
    case ImGuiDir_Right:
    ...
    g.CurrentWindow->DrawList->AddLine(center + a, center + b, GetColorU32(ImGuiCol_Text));
    g.CurrentWindow->DrawList->AddLine(center + a, center + c, GetColorU32(ImGuiCol_Text));
}
...
void ImGui::ItemSize(const ImVec2& size, float text_offset_y)
{
    ...
    window->DC.CurrentLineSize.y = window->DC.CurrLineTextBaseOffset = 0.0f;
...
void ImGui::UpdateMouseWheel()
{
    ...
    const float scroll_speed = ImFloor(window->CalcFontSize() * 10 * g.IO.DeltaTime + 10.5f);
    if (g.IO.MouseWheel != 0.0f)
    {
        ...
        else if (!g.IO.KeyCtrl && scroll_allowed)
        {
            ...
            SetWindowScrollY(scroll_window, scroll_window->Scroll.y - g.IO.MouseWheel * scroll_speed);
        }
    }
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    if (first_begin_of_the_frame)
    {
        ...
        else
        {
            ...
            if (flags & ImGuiWindowFlags_Popup)
            {
                window->DrawList->AddRectFilled(p, p + window->Size, ImColor(15, 15, 15, int(255 * style.Alpha)), style.FrameRounding);
                window->DrawList->AddRect(p, p + window->Size, ImColor(45, 45, 45, int(255 * style.Alpha)), style.FrameRounding);
            }

            // Window background
            if (!(flags & ImGuiWindowFlags_NoBackground) && !(flags & ImGuiWindowFlags_Popup))
            {
...
bool ImGui::ItemsToolTipBeginKeyBind(const char* name, bool hover, ImVec2 pos)
{
    SetNextWindowPos(ImVec2(pos.x, pos.y));
    OpenPopupOnItemClickKeyBind(name, hover);

    if (!BeginPopup(name, ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoScrollbar))
        return false;
...
{
    static bool openKeyBindPopup;
    ImGuiWindow* window = GImGui->CurrentWindow;
    if (IsMouseDown(mouse_button) && hovered && !openKeyBindPopup)
    {
        ImGuiID id = str_id ? window->GetID(str_id) : window->DC.LastItemId; // If user hasn't passed an ID, we can use the LastItemID. Using LastItemID as a Popup ID won't conflict!
        IM_ASSERT(id != 0);                                                  // You cannot pass a NULL str_id if the last item has no identifier (e.g. a Text() item)
        OpenPopupEx(id);
        openKeyBindPopup = true;
        return true;
    }
    else if (IsMouseClicked(mouse_button) && hovered && openKeyBindPopup)
    {
        openKeyBindPopup = false;
        CloseCurrentPopup();
        return false;
    }
    return false;
